...
type RootTarget = "branch" | "trunk" | "merge";
...
type RevHunkMergeStatus = "merged" | "unmerged" | "conflict" | "unknown";
...
type RevFileHunkStatusCache = {
  statuses: Array<RevHunkMergeStatus | undefined>;
  mergeRoot: string;
  mergeFileExists: boolean;
  mergeFileMtimeMs?: number;
};

type RevHunkLocateAttempt = {
  side: "old" | "new";
  strategy: "changed" | "full";
...
type RevHunkLocateResult = {
  lineNo: number;
  method: "match" | "fallback";
  attempts: RevHunkLocateAttempt[];
  mergeLineCount: number;
  matchedSide?: "old" | "new";
  matchedStrategy?: "changed" | "full";
  matchedExpectedLine?: number;
  matchedLine?: number;
};

type RevHunkTextMergeCheck = {
  mergedByText: boolean;
  oldSnippetLines: number;
  newSnippetLines: number;
  oldMatches: number[];
  newMatches: number[];
  relaxedUsed: boolean;
  keyLineTotal: number;
  keyLineMatched: number;
  keyWindowAnchor?: number;
  keyWindowRadius: number;
  keyHitRatio: number;
};

type RevTextApplyResult = {
  ok: boolean;
  alreadyMerged?: boolean;
  mode: "add" | "delete" | "replace" | "none";
  message: string;
  detail?: string;
};

type RevHunkTextApplyCheck = {
  applicable: boolean;
  mode: "add" | "delete" | "replace" | "none";
  reason:
    | "none"
...
type EnsureSvnTrackedResult = {
  ok: boolean;
  action: "skipped-missing" | "already-tracked" | "added" | "failed";
  message: string;
  statusOutput?: string;
  addOutput?: string;
};

type DebugFileSnapshot = {
  path: string;
  exists: boolean;
  isFile: boolean;
  size?: number;
...
type RevChangeState = {
  ...
  diffByPath: Map<string, RevFileDiff>;
  hunkStatusByPath: Map<string, RevFileHunkStatusCache>;
  mergeSummaryByPath: Map<string, RevFileMergeSummary>;
};

type DiffCompareTarget = "merge" | "branch" | "trunk";
type DiffCompareStatus = "matched" | "missing" | "extra" | "mismatch";
type DiffCompareEntry = {
  path: string;
  rangeStatus?: string;
  localStatus?: string;
  status: DiffCompareStatus;
};
type DiffCompareSummary = {
  matched: number;
  missing: number;
  extra: number;
  mismatch: number;
...
type DiffCompareState = {
  target: DiffCompareTarget;
  remoteRev: string;
  localItems: RevChangeItem[];
  compareItems: DiffCompareEntry[];
...
class RootPathItem extends vscode.TreeItem {
  target: RootTarget;
  rootPath?: string;
  constructor(target: RootTarget, rootPath?: string) {
...
class RootsGroupItem extends vscode.TreeItem {
  children: RootPathItem[];
  constructor(roots?: AnalysisRoots) {
...
function formatDiffCompareTarget(target: DiffCompareTarget) {
  if (target === "merge") return "待合并";
  if (target === "branch") return "分支";
  return "主线";
}

function normalizePathKey(relPath: string) {
  return relPath.replace(/\\/g, "/").toLowerCase();
}

function getTreeItemPathParts(relPath: string) {
...
function buildDiffCompareGroup(
  label: string,
  iconId: string,
  entries: DiffCompareEntry[],
  getDesc: (entry: DiffCompareEntry) => string
) {
  ...
  group.children = entries
    ...
    .map(
      (entry) =>
        new DiffCompareFileItem(entry.path, getDesc(entry), entry.status)
    );
  return group;
}

function compareDiffItems(
  rangeItems: RevChangeItem[],
  localItems: RevChangeItem[]
) {
  const rangeMap = new Map<string, RevChangeItem>();
  const localMap = new Map<string, RevChangeItem>();
  ...
  const keys = new Set<string>([...rangeMap.keys(), ...localMap.keys()]);
  const entries: DiffCompareEntry[] = [];
  let matched = 0;
  let missing = 0;
  let extra = 0;
  let mismatch = 0;
  ...
  const summary: DiffCompareSummary = {
    matched,
    missing,
    extra,
    mismatch,
...
function formatRevMergeSummaryText(summary?: RevFileMergeSummary) {
...
function getRevFileIcon(status: string, summary?: RevFileMergeSummary) {
...
class RevFileItem extends vscode.TreeItem {
  relPath: string;
  status: string;
  mergeSummary?: RevFileMergeSummary;
  constructor(relPath: string, status: string, mergeSummary?: RevFileMergeSummary) {
...
class DiffCompareFileItem extends vscode.TreeItem {
  relPath: string;
  status: DiffCompareStatus | "local";
  constructor(relPath: string, description: string, status: DiffCompareStatus | "local") {
    const pathParts = getTreeItemPathParts(relPath);
    super(pathParts.fileName, vscode.TreeItemCollapsibleState.None);
    this.relPath = relPath;
    this.status = status;
...
class DiffCompareTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<
    vscode.TreeItem | undefined | void
  > = new vscode.EventEmitter<vscode.TreeItem | undefined | void>();
  readonly onDidChangeTreeData: vscode.Event<vscode.TreeItem | undefined | void> =
  ...
  refresh(state?: DiffCompareState) {
    if (!state) {
      this.nodes = [
    ...
    const summary = new DiffCompareSummaryItem(
      `范围: ${state.rangeLabel} | 目标: ${formatDiffCompareTarget(state.target)} | 远程: ${state.remoteRev}`,
      summaryText
    );

    const localGroup = new DiffCompareGroupItem(
      "本地差异",
      `${state.localItems.length}项`,
      "diff"
    );
    localGroup.children = state.localItems
      ...
      .map(
        (item) =>
          new DiffCompareFileItem(
            item.path,
            normalizeRevStatus(item.status),
            "local"
          )
      );

    const matchedGroup = buildDiffCompareGroup(
      "已覆盖",
      "check",
      state.compareItems.filter((item) => item.status === "matched"),
      (item) => `范围:${item.rangeStatus || "-"} 本地:${item.localStatus || "-"}`
    );
    const missingGroup = buildDiffCompareGroup(
      "缺失",
      "error",
      state.compareItems.filter((item) => item.status === "missing"),
      (item) => `范围:${item.rangeStatus || "-"} 本地:-`
    );
    const extraGroup = buildDiffCompareGroup(
      "多余",
      "add",
      state.compareItems.filter((item) => item.status === "extra"),
      (item) => `范围:- 本地:${item.localStatus || "-"}`
    );
    const mismatchGroup = buildDiffCompareGroup(
      "不一致",
      "warning",
      state.compareItems.filter((item) => item.status === "mismatch"),
      (item) => `范围:${item.rangeStatus || "-"} 本地:${item.localStatus || "-"}`
    );
...
function formatRootLabel(target: RootTarget) {
  if (target === "branch") return "分支";
  if (target === "trunk") return "主线";
  return "待合并";
}
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let diffCompareState: DiffCompareState | undefined;
    let revMergeSummaryScanToken = 0;
    const revRootCache = new Map<RevRangeTarget, RootInfo>();
    const diffCompareRootCache = new Map<DiffCompareTarget, RootInfo>();
    const revFileLogCache = new Map<string, RevFileLogEntry[]>();
    const fileHistoryCompareCache = new Map<string, RevFileLogEntry[]>();
  ...
  const statusBarPluginName = "SVN Merge Annotator";
  const statusBarTitle = "SVN合并后端";
  const statusBarLogo = "$(git-merge)";
  statusBar.command = "svnMergeAnnotator.backendActions";
  statusBar.name = `${statusBarPluginName} 后端状态`;
  ...
  statusBar.show();
    ...
    const diffCompareView = vscode.window.createTreeView(
      "svnMergeAnnotator.diffCompare",
      {
  ...
  const diffLensProvider: vscode.CodeLensProvider = {
    onDidChangeCodeLenses: diffLensEmitter.event,
    async provideCodeLenses(document) {
      if (document.uri.scheme !== "svnrev") return [];
  ...
  context.subscriptions.push(
    ...
    vscode.window.onDidChangeActiveTextEditor((editor) => {
      if (!editor || editor.document.uri.scheme !== "svnrev") return;
  ...
  const revCompareContentProvider: vscode.TextDocumentContentProvider = {
    provideTextDocumentContent: async (uri) => {
      const relPath = decodeURIComponent(uri.path.replace(/^\/+/, ""));
      const params = new URLSearchParams(uri.query);
      const rev = params.get("rev") || "";
      ...
      const target = targetRaw as RevRangeTarget;
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.registerTextDocumentContentProvider(
      SVN_REV_COMPARE_SCHEME,
      revCompareContentProvider
    )
  );
  ...
  function captureFileSnapshotForDebug(filePath: string): DebugFileSnapshot {
    try {
      ...
      const snapshot: DebugFileSnapshot = {
        path: filePath,
        exists: true,
        isFile: stat.isFile(),
        size: stat.size,
  ...
  function setBackendStatus(status: BackendStatus, detail?: string) {
    ...
    let text = `${statusBarLogo} $(question) ${statusBarTitle}: 未知`;
    switch (status) {
      case "starting":
        text = `${statusBarLogo} $(sync~spin) ${statusBarTitle}: 启动中`;
        break;
      case "running":
        text = `${statusBarLogo} $(play-circle) ${statusBarTitle}: 运行中`;
        break;
      case "stopped":
        text = `${statusBarLogo} $(circle-slash) ${statusBarTitle}: 未启动`;
        break;
      case "error":
        text = `${statusBarLogo} $(error) ${statusBarTitle}: 异常`;
        break;
      default:
        break;
    }
  ...
  async function ensureAnnotationFilterAll(source?: string) {
    ...
    try {
      ...
      logMessage("INFO", "annotationFilter reset to all", {
        from: current,
        source: source || "unknown",
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      logMessage("WARN", "annotationFilter reset failed", {
        error: message,
        source: source || "unknown",
      });
    }
  ...
  function buildDisplayRoots(): AnalysisRoots {
  ...
  function refreshFilesTree() {
    treeProvider.refresh(state.files || [], state.summaryByPath, buildDisplayRoots());
  }
  ...
  state.notesLoaded = false;
    ...
    diffCompareProvider.refresh(diffCompareState);
    await ensureAnnotationFilterAll("loadFiles");
    applyFileFilters();
  ...
  function getKnownMergeRoot(): string | undefined {
    if (state.roots?.merge) return state.roots.merge;
    const lastDirs = context.globalState.get<LastDirs>(LAST_DIRS_KEY);
    if (lastDirs?.merge) {
      state.roots = state.roots || {};
      state.roots.merge = lastDirs.merge;
  ...
  async function updateRootPath(target: RootTarget, value: string) {
  ...
  async function setRootPath(item?: RootPathItem | RootTarget) {
  ...
  async function copyRootPath(item?: RootPathItem | RootTarget) {
    const target = resolveRootTarget(item);
    if (!target) {
      vscode.window.showErrorMessage("未选择路径类型");
      return;
  ...
  async function clearRootPath(item?: RootPathItem | RootTarget) {
    const target = resolveRootTarget(item);
    if (!target) {
      vscode.window.showErrorMessage("未选择路径类型");
      return;
  ...
  async function openMergeFile(
    ...
    if (loadDetail && !state.analysisId) {
      ...
      group.children = groupItems.map(
        (item) =>
          new RevFileItem(
            item.path,
            normalizeRevStatus(item.status),
            mergeSummaryByPath?.get(item.path)
          )
      );
      nodes.push(group);
    }
...
function buildRevNodes(state: RevChangeState) {
  ...
  async function getRootInfoForCompare(
    target: DiffCompareTarget
  ): Promise<RootInfo | undefined> {
    const cached = diffCompareRootCache.get(target);
    if (cached) return cached;
    const rootPath =
      target === "merge" ? await resolveMergeRoot() : await resolveRoot(target);
    if (!rootPath) return undefined;
    const info = await runSvn(["info", "--xml", rootPath], rootPath);
    if (info.code !== 0) {
      throw new Error(info.stderr || "无法读取 SVN 信息");
  ...
  function parseSummarizeOutput(output: string, root: RootInfo) {
  ...
  async function fetchLocalDiffItems(root: RootInfo, remoteRev: string) {
    const rev = normalizeRevInput(remoteRev) || "HEAD";
    const peg = `${root.rootUrl}@${rev}`;
    const resp = await runSvn(
      ["diff", "--summarize", "--old", peg, "--new", root.rootPath],
      root.rootPath
    );
    if (resp.code !== 0) {
  ...
  function parseFileLogEntriesFromXml(xml: string) {
  ...
  async function fetchFileLogEntriesLatest(
    root: RootInfo,
    relPath: string,
    limit = FILE_HISTORY_PICK_LIMIT
  ) {
    const fileUrl = joinUrl(root.rootUrl, relPath);
    const resp = await runSvn(
      ["log", "--xml", "-l", String(limit), fileUrl],
      root.rootPath
    );
    if (resp.code !== 0) {
  ...
  async function runDiffCompareInternal(
    target: DiffCompareTarget,
    remoteRev: string
  ) {
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        const { entries, summary } = compareDiffItems(
          currentRevState.diffItems || [],
          localItems
        );
        const rangeLabel = `${formatRevLabel(
          currentRevState.startRev
        )} → ${formatRevLabel(currentRevState.endRev)} (${
          currentRevState.target === "branch" ? "分支" : "主线"
        })`;
        diffCompareState = {
          target,
  ...
  async function compareDiffs() {
    ...
    type TargetPick = vscode.QuickPickItem & { value: DiffCompareTarget };
    const lastTarget =
      context.globalState.get<DiffCompareTarget>(DIFF_COMPARE_TARGET_KEY) ||
      "merge";
    const targetItems: TargetPick[] = [
    ...
    const targetPick = await vscode.window.showQuickPick<TargetPick>(targetItems, {
      placeHolder: "选择要对比的本地目录",
    });
    if (!targetPick) return;
    const lastRemote =
      context.globalState.get<string>(DIFF_COMPARE_REMOTE_REV_KEY) || "HEAD";
  ...
  async function refreshDiffCompare() {
    ...
    const lastRemote =
      context.globalState.get<string>(DIFF_COMPARE_REMOTE_REV_KEY) ||
  ...
  function parseFirstHunkStart(diffText: string): number | undefined {
  ...
  function parseRevHunkHeader(line: string) {
  ...
  function parseRevFileDiff(diffText: string): RevFileDiff {
    const lines = diffText.split(/\r?\n/);
    const headers: string[] = [];
    const hunks: RevDiffHunk[] = [];
    let currentHunk: RevDiffHunk | undefined;
    for (const line of lines) {
      if (line.startsWith("@@")) {
  ...
  function buildSingleHunkPatch(diff: RevFileDiff, hunk: RevDiffHunk) {
    const lines = [...diff.headers, ...hunk.lines];
    const body = lines.join("\n").replace(/\n+$/, "");
    return `${body}\n`;
  }

  function normalizeRevCompareValue(value?: string) {
    if (!value) return "";
  ...
  function resolveRevHunkByLine(
    hunks: RevDiffHunk[],
    lineNo: number,
    useOldSide: boolean
  ): RevDiffHunk | undefined {
    if (!hunks.length) return undefined;
    let nearest: RevDiffHunk | undefined;
  ...
  function getRevCachePathKey(relPath: string) {
    return normalizeSlashes(relPath);
  }

  function normalizeMergeRelPath(relPath: string) {
    return normalizeSlashes(relPath).replace(/^\/+/, "");
  }

  function buildMergeRelPathCandidates(relPath: string) {
    ...
    const rootBase = normalizeSlashes(
      path.basename(path.normalize(revState.root.rootPath || ""))
    ).replace(/^\/+|\/+$/g, "");
    if (rootBase && rootBase !== ".") {
      prefixes.add(rootBase);
    }
    const suffixRaw = normalizeSlashes(revState.root.rootSuffix || "").replace(
      /^\/+|\/+$/g,
      ""
    );
    if (suffixRaw) {
      ...
      if (
        (segments[0] === "branches" || segments[0] === "tags") &&
        segments.length >= 3
      ) {
  ...
  function scoreMergeRelPathCandidate(mergeRoot: string, candidate: string) {
  ...
  function resolveMergePathContext(mergeRoot: string, relPath: string) {
    ...
    const filePath = path.normalize(
      path.join(applyRoot, normalizedRelPath.replace(/\//g, path.sep))
    );
    return {
      normalizedRelPath,
  ...
  async function ensureRevFileDiff(
    relPath: string,
    force = false
  ): Promise<RevFileDiff> {
    if (!revState) {
    ...
    const diffResp = await runSvn(
      ["diff", "-r", range, fileUrl],
      revState.root.rootPath
    );
    if (diffResp.code !== 0) {
  ...
  function getMergeFileSnapshot(mergeRoot: string, relPath: string) {
    const mergePath = resolveMergePathContext(mergeRoot, relPath);
    const filePath = mergePath.filePath;
    if (!fs.existsSync(filePath)) {
      return { filePath, exists: false, mtimeMs: undefined as number | undefined };
    }
    try {
      const stat = fs.statSync(filePath);
      if (!stat.isFile()) {
  ...
  function isRevHunkStatusCacheValid(
    cache: RevFileHunkStatusCache,
    mergeRoot: string,
    mergeFileExists: boolean,
    mergeFileMtimeMs?: number
  ) {
    if (cache.mergeRoot !== mergeRoot) return false;
    if (cache.mergeFileExists !== mergeFileExists) return false;
    return cache.mergeFileMtimeMs === mergeFileMtimeMs;
  }

  async function runPatchWithText(
    relPath: string,
    patchText: string,
    mergeRoot: string,
    dryRun: boolean,
    ...
    const patchPath = buildTempPatchPath(
      `${relPath}-${dryRun ? "dry" : "apply"}-${reverse ? "reverse" : "forward"}`
    );
    try {
  ...
  async function detectRevHunkMergeStatus(
    relPath: string,
    diff: RevFileDiff,
    hunk: RevDiffHunk,
    mergeRoot: string
  ): Promise<RevHunkMergeStatus> {
    if (!hunk.lines.length) return "unknown";
  ...
  function createDefaultRevFileMergeSummary(): RevFileMergeSummary {
    return {
      total: 0,
      merged: 0,
      unmerged: 0,
  ...
  async function ensureRevHunkStatus(
    relPath: string,
    hunkIndex: number,
    force = false
  ): Promise<RevHunkMergeStatus> {
    if (!revState) return "unknown";
    const key = getRevCachePathKey(relPath);
    ...
    if (!activeCache) return "unknown";
    if (!force && activeCache.statuses[hunkIndex] !== undefined) {
      return activeCache.statuses[hunkIndex] || "unknown";
    }
    let status: RevHunkMergeStatus = "unknown";
    try {
      status = await detectRevHunkMergeStatus(
        key,
        diff,
  ...
  async function ensureRevFileMergeSummary(
    relPath: string,
    force = false
  ): Promise<RevFileMergeSummary> {
    if (!revState) {
  ...
  function invalidateRevPathMergeCache(relPath: string) {
    if (!revState) return;
    const key = getRevCachePathKey(relPath);
    revState.hunkStatusByPath.delete(key);
    revState.mergeSummaryByPath.delete(key);
  ...
  function getRevHunkStatusLabel(status: RevHunkMergeStatus) {
    if (status === "merged") return "已合并";
    if (status === "unmerged") return "未合并";
    if (status === "conflict") return "冲突";
    return "未知";
  }

  function buildRevHunkSnippetLines(hunk: RevDiffHunk, side: "old" | "new") {
  ...
  function buildRevHunkChangedLines(hunk: RevDiffHunk, side: "old" | "new") {
    const lines: string[] = [];
    for (let i = 1; i < hunk.lines.length; i += 1) {
      const line = hunk.lines[i];
      if (!line) continue;
  ...
  function buildRevHunkChangedRawLines(hunk: RevDiffHunk, side: "old" | "new") {
    const lines: string[] = [];
    for (let i = 1; i < hunk.lines.length; i += 1) {
      const line = hunk.lines[i];
      if (!line) continue;
  ...
  function getRevHunkChangeMode(hunk: RevDiffHunk): "add" | "delete" | "replace" | "none" {
    const oldLines = buildRevHunkChangedRawLines(hunk, "old");
    const newLines = buildRevHunkChangedRawLines(hunk, "new");
    if (!oldLines.length && !newLines.length) return "none";
    if (!oldLines.length) return "add";
    if (!newLines.length) return "delete";
    return "replace";
  }

  function getFirstPlusLineByOldSpace(hunk: RevDiffHunk) {
    let oldLine = hunk.oldStart;
    for (let i = 1; i < hunk.lines.length; i += 1) {
      const line = hunk.lines[i] || "";
      if (line.startsWith("+")) return Math.max(1, oldLine || 1);
  ...
  function getFirstPlusLineByNewSpace(hunk: RevDiffHunk) {
    let newLine = hunk.newStart;
    for (let i = 1; i < hunk.lines.length; i += 1) {
      const line = hunk.lines[i] || "";
      if (line.startsWith("+")) return Math.max(1, newLine || 1);
      if (line.startsWith(" ") || line.startsWith("+")) {
        newLine += 1;
      }
    }
  ...
  function normalizeLooseSnippetSequence(lines: string[]) {
    return lines.map(normalizeStrictSnippetLine).filter((line) => !!line);
  }

  function findLooseSequenceRanges(snippetLines: string[], targetLines: string[]) {
    const snippet = normalizeLooseSnippetSequence(snippetLines);
    if (!snippet.length || !targetLines.length) return [];
    ...
    if (size > compactTarget.length) return [];
  ...
  function normalizeLooseLine(value: string) {
    return value.trim().replace(/\s+/g, " ");
  }

  function isMeaningfulKeyLine(line: string) {
    const normalized = normalizeLooseLine(line);
    if (!normalized) return false;
    if (normalized === "{" || normalized === "}") return false;
    if (normalized === "/// <summary>" || normalized === "/// </summary>") return false;
    if (normalized.length < 6) return false;
    if (!/[A-Za-z0-9_\u4e00-\u9fa5]/.test(normalized)) return false;
    return true;
  }

  function findLooseLineMatches(line: string, targetLines: string[]) {
  ...
  function evaluateKeyLineWindowMatch(
    snippetLines: string[],
    targetLines: string[],
    windowRadius = 80
  ) {
    ...
    const anchorCandidates = Array.from(
      new Set(keyMatches.flatMap((items) => items))
    ).slice(0, 500);
    let bestMatched = 0;
  ...
  function evaluateRevHunkTextMerge(
    targetLines: string[],
    hunk: RevDiffHunk
  ): RevHunkTextMergeCheck {
    ...
    let keyWindowAnchor: number | undefined;
  ...
  function evaluateRevHunkTextApply(
    targetLines: string[],
    hunk: RevDiffHunk
  ): RevHunkTextApplyCheck {
  ...
  function pickNearestLine(matches: number[], expected: number) {
  ...
  async function resolveMergeLineByHunkContextDetailed(
    filePath: string,
    hunk: RevDiffHunk,
    status: RevHunkMergeStatus
  ): Promise<RevHunkLocateResult> {
    const mergeDoc = await vscode.workspace.openTextDocument(filePath);
    const targetLines = mergeDoc.getText().split(/\r?\n/);
    const attempts: RevHunkLocateAttempt[] = [];
    const sideOrder: Array<"old" | "new"> =
      status === "merged" ? ["new", "old"] : ["old", "new"];
    for (const side of sideOrder) {
      const candidates: Array<{ strategy: "changed" | "full"; snippet: string[] }> = [
      ...
      for (const candidate of candidates) {
        ...
        const attempt: RevHunkLocateAttempt = {
          side,
  ...
  async function resolveMergeLineByHunkContext(
    filePath: string,
    hunk: RevDiffHunk,
    status: RevHunkMergeStatus
  ) {
  ...
  async function tryApplyRevHunkByText(
    relPath: string,
    hunk: RevDiffHunk,
    filePath: string
  ): Promise<RevTextApplyResult> {
    const mode = getRevHunkChangeMode(hunk);
    if (mode === "none") {
      return {
        ok: false,
    ...
    if (mode === "add") {
      ...
      const locate = await resolveMergeLineByHunkContextDetailed(
        filePath,
        hunk,
        "unmerged"
      );
  ...
  function summarizeLogText(raw: string, maxLines = 8) {
  ...
  function buildHunkHeaderText(hunk: RevDiffHunk) {
    return `@@ -${hunk.oldStart},${hunk.oldCount} +${hunk.newStart},${hunk.newCount} @@${
      hunk.section ? ` ${hunk.section}` : ""
    }`;
  }

  async function buildRevHunkDebugLog(relPath: string, hunkIndex: number) {
    if (!revState) {
      throw new Error("尚未运行提交范围变更");
    }
    const key = getRevCachePathKey(relPath);
    ...
    lines.push(`文件: ${key}`);
    lines.push(
      `范围: ${formatRevLabel(revState.startRev)} -> ${formatRevLabel(revState.endRev)}`
    );
    ...
    const patchText = rewritePatchPaths(
      buildSingleHunkPatch(diff, hunk),
      revState.root.rootUrl
    );
    ...
    lines.push(
      `DryRunForward: ok=${forward.ok} code=${forward.code} conflict=${forward.conflict} skipped=${forward.skipped} alreadyApplied=${forward.alreadyApplied}`
    );
    lines.push(
      `DryRunReverse: ok=${reverse.ok} code=${reverse.code} conflict=${reverse.conflict} skipped=${reverse.skipped} alreadyApplied=${reverse.alreadyApplied}`
    );
    ...
    if (snapshot.exists) {
      try {
        ...
        const textApply = evaluateRevHunkTextApply(targetLines, hunk);
        lines.push(
          `TextFallback: merged=${textCheck.mergedByText} oldLines=${textCheck.oldSnippetLines} newLines=${textCheck.newSnippetLines} relaxed=${
            textCheck.relaxedUsed
          } keyMatched=${textCheck.keyLineMatched}/${textCheck.keyLineTotal} keyRatio=${textCheck.keyHitRatio.toFixed(
            2
          )} keyAnchor=${
            textCheck.keyWindowAnchor || "-"
          } keyRadius=${textCheck.keyWindowRadius} oldMatches=${
            textCheck.oldMatches.length ? textCheck.oldMatches.join(",") : "(none)"
          } newMatches=${
            textCheck.newMatches.length ? textCheck.newMatches.join(",") : "(none)"
          }`
        );
        lines.push(
          `TextApplyable: canApply=${textApply.applicable} mode=${textApply.mode} reason=${textApply.reason}`
        );
      } catch (err) {
    ...
    if (snapshot.exists) {
      ...
      lines.push(
        `Locate: line=${locate.lineNo} method=${locate.method} mergeLines=${locate.mergeLineCount}`
      );
      for (const attempt of locate.attempts) {
        const matchesText = attempt.matches.length
          ...
          : "(none)";
        lines.push(
          `  - side=${attempt.side} strategy=${attempt.strategy} snippet=${attempt.snippetLineCount} expected=${attempt.expectedLine} picked=${
            attempt.pickedLine || "-"
          } matches=${matchesText}`
        );
      }
  ...
  async function resolveCurrentRevHunkContext(
    ...
    | undefined
  > {
    if (!revState) return undefined;
    if (document.uri.scheme !== "svnrev") return undefined;
    const relPath = decodeURIComponent(document.uri.path.replace(/^\/+/, ""));
    const rev = new URLSearchParams(document.uri.query).get("rev") || "";
    const useOldSide =
      normalizeRevCompareValue(rev) ===
  ...
  async function refreshRevMergeSummariesInBackground(
    targetPaths?: string[],
    force = false
  ) {
    if (!revState) return;
  ...
  function findRevStatusByPath(relPath: string): string | undefined {
    if (!revState?.diffItems?.length) return undefined;
    const normalized = normalizeSlashes(relPath);
    const match = revState.diffItems.find(
      (item) => normalizeSlashes(item.path) === normalized
    );
    return match?.status;
  }

  function normalizeSnippetLine(value: string) {
    return value.replace(/\s+/g, "");
  }

  function trimSnippetLines(lines: string[]) {
    let start = 0;
  ...
  function findSnippetMatches(
    snippetLines: string[],
    targetLines: string[]
  ): number[] {
    if (!snippetLines.length || !targetLines.length) return [];
  ...
  function normalizeStrictSnippetLine(value: string) {
    return value.replace(/\s+/g, " ").trim();
  }

  function findExactSnippetMatches(
    snippetLines: string[],
    targetLines: string[]
  ): number[] {
    if (!snippetLines.length || !targetLines.length) return [];
    const normalizedSnippet = snippetLines
      ...
      .filter((line) => !!line);
    if (!normalizedSnippet.length) return [];
    const compactTarget = targetLines
      .map((line, idx) => ({
        text: normalizeStrictSnippetLine(line || ""),
        lineNo: idx + 1,
  ...
  function hasPatchConflict(output: string): boolean {
    if (!output) return false;
    const lines = output.split(/\r?\n/);
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      if (/^C\s/.test(trimmed)) return true;
    }
    if (output.includes(".svnpatch.rej")) return true;
    return false;
  }

  function hasPatchSkipped(output: string): boolean {
    if (!output) return false;
    const lines = output.split(/\r?\n/);
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      if (/^Skipped\s+/i.test(trimmed)) return true;
      if (/^>\s*Skipped\s+/i.test(trimmed)) return true;
      if (/^Summary of conflicts:\s*$/i.test(trimmed)) continue;
      if (/^Skipped paths:\s*\d+/i.test(trimmed)) return true;
      if (/obstructed by unversioned node/i.test(trimmed)) return true;
    }
    return false;
  }

  function hasPatchAlreadyApplied(output: string): boolean {
    if (!output) return false;
    const lines = output.split(/\r?\n/);
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      if (/already applied/i.test(trimmed)) return true;
      if (/hunk .*already applied/i.test(trimmed)) return true;
    }
    return false;
  }

  function hasSvnAlreadyVersioned(output: string): boolean {
    if (!output) return false;
    return (
      /already under version control/i.test(output) ||
  ...
  function hasSvnUnversionedStatus(output: string): boolean {
    if (!output) return false;
    const lines = output.split(/\r?\n/);
    for (const line of lines) {
  ...
  async function ensureSvnTracked(
    targetPath: string,
    opId: string,
    reason: string
  ): Promise<EnsureSvnTrackedResult> {
    if (!targetPath) {
      return {
        ok: false,
        action: "failed",
    ...
    const unversioned =
      statusResp.code === 0 ? hasSvnUnversionedStatus(statusResp.stdout) : true;
    ...
    const addResp = await runSvn(
      ["add", "--parents", "--force", targetPath],
      cwd
    );
  ...
  function buildTempPatchPath(relPath: string) {
    const safeName = relPath.replace(/[\\\/:]+/g, "_");
    const filename = `svn-merge-annotator-${Date.now()}-${safeName}.patch`;
    return path.join(os.tmpdir(), filename);
  }

  function rewritePatchPaths(diffText: string, rootUrl: string): string {
    if (!diffText || !rootUrl) return diffText;
  ...
  async function ensureMergeFileReady(
    relPath: string,
    status?: string
  ): Promise<{ mergeRoot: string; filePath: string } | undefined> {
    const mergeRoot = await resolveMergeRoot();
    if (!mergeRoot) {
      vscode.window.showErrorMessage("缺少 merge 根目录信息");
      return undefined;
  ...
  async function resolveRevDiffStartLine(
    relPath: string,
    startRev: string,
    endRev: string
  ...
  async function openRevMergeFile(item?: RevFileItem) {
    if (!item) return;
    if (!revState) {
      vscode.window.showErrorMessage("尚未运行提交范围变更");
      return;
    ...
    try {
      lineNo = await resolveRevDiffStartLine(
        item.relPath,
        revState.startRev,
        revState.endRev
      );
  ...
  async function resolveActiveRevHunkInput() {
    ...
    const context = await resolveCurrentRevHunkContext(
      editor.document,
  ...
  async function copyRevHunkDebugLog(relPath?: string, hunkIndex?: number) {
    ...
    if (!resolvedPath || resolvedIndex === undefined) return;
    try {
  ...
  async function openMergeByHunkContext(relPath?: string, hunkIndex?: number) {
    if (!revState) {
      vscode.window.showErrorMessage("尚未运行提交范围变更");
      return;
    }
  ...
  async function applyRevHunkToMerge(relPath?: string, hunkIndex?: number) {
    if (!revState) {
      vscode.window.showErrorMessage("尚未运行提交范围变更");
      return;
    }
    ...
    const busy = vscode.window.setStatusBarMessage(
      "$(sync~spin) SVN Merge Annotator: 正在合并变更块..."
    );
    ...
    try {
      ...
      const dryRun = await runPatchWithText(
        key,
        patchText,
        mergeRoot,
        true,
      ...
      const forceTextFallback =
        hunkMode === "add" && !targetFileExists && !!dryRun.alreadyApplied;
      ...
      if (!dryRun.ok || forceTextFallback) {
        let textApply: RevTextApplyResult | undefined;
        try {
        ...
        if (textApply.ok) {
          let svnTrackResult: EnsureSvnTrackedResult | undefined;
          if (hunkMode === "add") {
            svnTrackResult = await ensureSvnTracked(
              targetPath,
              opId,
              "applyRevHunkToMerge:text-fallback"
            );
            if (!svnTrackResult.ok) {
              vscode.window.showWarningMessage(
                `变更块已应用，但加入 SVN 跟踪失败：${svnTrackResult.message}`
              );
            }
          }
          ...
          if (finalStatus === "merged") {
            vscode.window.showInformationMessage(
              textApply.alreadyMerged
                ? textApply.message || "该变更块已合并（文本匹配判定）"
                : textApply.message || "该变更块已通过文本匹配应用"
            );
          } else {
            vscode.window.showWarningMessage(
              `已执行文本合并，但当前状态为${getRevHunkStatusLabel(finalStatus)}，请点“复制日志”排查`
            );
          }
      ...
      const applyResp = await runPatchWithText(
        key,
        patchText,
        mergeRoot,
        false,
      ...
      if (hunkMode === "add") {
        svnTrackResult = await ensureSvnTracked(
          targetPath,
          opId,
          "applyRevHunkToMerge:patch-apply"
        );
        if (!svnTrackResult.ok) {
          vscode.window.showWarningMessage(
            `变更块已应用，但加入 SVN 跟踪失败：${svnTrackResult.message}`
      ...
      if (finalStatus === "merged") {
        ...
        vscode.window.showWarningMessage(
          `补丁执行完成，但当前状态为${getRevHunkStatusLabel(finalStatus)}，请点“复制日志”排查`
        );
  ...
  async function openMergeBySelection() {
    ...
    if (matches.length > 1) {
      type Pick = vscode.QuickPickItem & { lineNo: number };
      const picks: Pick[] = matches.slice(0, 200).map((line) => ({
        label: `L${line}`,
  ...
  async function applyRevChangeToMergeByPath(
    relPath: string,
    status: string
  ): Promise<boolean> {
    if (!revState) {
    ...
    const busy = vscode.window.setStatusBarMessage(
      "$(sync~spin) SVN Merge Annotator: 正在一键更新到待合并..."
    );
    ...
    try {
      ...
      const normalizedPatch = rewritePatchPaths(
        diffResp.stdout,
        revState.root.rootUrl
      );
      ...
      try {
        ...
        const dryResp = await runSvn(
          ["patch", "--dry-run", patchPath, patchRoot],
          patchRoot
        );
          ...
          if (normalizedStatus === "A" && (drySkipped || forceTextFallback)) {
            ...
            if (parsed.hunks.length > 0) {
              ...
              for (const hunk of parsed.hunks) {
                const textApply = await tryApplyRevHunkByText(
                  relPath,
                  hunk,
                  mergePath.filePath
                );
              ...
              if (allOk) {
                ...
                if (normalizedStatus === "A") {
                  svnTrackResult = await ensureSvnTracked(
                    mergePath.filePath,
                    opId,
                    "applyRevChangeToMergeByPath:text-fallback"
                  );
                  if (!svnTrackResult.ok) {
        ...
        if (normalizedStatus === "A") {
          svnTrackResult = await ensureSvnTracked(
            mergePath.filePath,
            opId,
            "applyRevChangeToMergeByPath:patch-apply"
          );
          if (!svnTrackResult.ok) {
            const message = `新增文件已应用，但加入 SVN 跟踪失败：${svnTrackResult.message}`;
            vscode.window.showErrorMessage(message);
        ...
        if (summary.total > 0 && summary.merged === 0 && summary.unmerged === summary.total) {
          vscode.window.showWarningMessage(
            "补丁执行完成，但该文件仍未合并任何变更块，请检查路径映射或点“复制日志”排查"
          );
  ...
  async function applyRevChangeToMerge(item?: RevFileItem) {
    if (!item) {
  ...
  function resolveCompareFilePath(
    input?: FileItem | RevFileItem | vscode.Uri | string
  ) {
    ...
    if (input instanceof vscode.Uri) {
      ...
      const tryResolveByRoot = (rootDir?: string) => {
  ...
  function formatCompareSourceLabel(source: CompareSourceTarget) {
    if (source === "local") {
      return "本地";
    }
  ...
  async function pickCompareSource(
    placeHolder: string,
    preferred?: CompareSourceTarget
  ) {
    type SourcePick = vscode.QuickPickItem & { value: CompareSourceTarget };
  ...
  async function pickFileRevisionForCompare(
    relPath: string,
    target: CompareSourceTarget,
    sideLabel: string
  ) {
    if (target === "local") {
    ...
    if (!entries) {
      entries = await fetchFileLogEntriesLatest(
        root,
        relPath,
        FILE_HISTORY_PICK_LIMIT
      );
    ...
    type RevPick = vscode.QuickPickItem & { revision: string };
    const picks: RevPick[] = entries.map((entry) => {
      const message = formatLogMessage(entry.message);
      const detailParts = [];
  ...
  async function openCompareRevDiffForPath(
    relPath: string,
    leftTarget: CompareSourceTarget,
    leftRev: string,
    rightTarget: CompareSourceTarget,
    rightRev: string
  ) {
    ...
    const title = `${safeRelPath} (左:${formatRootLabel(
      leftTarget
    )} ${formatRevLabel(leftRev)} → 右:${formatRootLabel(
      rightTarget
    )} ${formatRevLabel(rightRev)})`;
    await vscode.commands.executeCommand("vscode.diff", left, right, title);
  }

  async function pickLocalFileForCompare(relPath: string, sideLabel: string) {
    const enableEnhancedLocalPick = context.globalState.get<boolean>(
      "compare.localFilePicker.enhanced",
      true
    );
    ...
    const excludePattern =
      "**/{.git,.svn,node_modules,dist,build,out,.idea,.vscode}/**";
    const preferredIncludePattern = targetFileName
      ? `**/${targetFileName}`
      : "**/*";
    let candidates = await vscode.workspace.findFiles(
      preferredIncludePattern,
      excludePattern,
      800
    );
    if (targetFileName) {
      candidates = candidates.filter(
        (uri) => path.basename(uri.fsPath).toLowerCase() === targetFileName
      );
    }
    if (!candidates.length && targetFileName) {
      const allCandidates = await vscode.workspace.findFiles(
        "**/*",
        excludePattern,
        3000
      );
    ...
    type LocalFilePick = vscode.QuickPickItem & {
      fsPath: string;
      score: number;
      sortKey: string;
    };
    const normalizedRelLower = normalizedRelPath.toLowerCase();
    const picks: LocalFilePick[] = candidates.map((uri) => {
  ...
  async function pickLocalFileForCompareEnhanced(relPath: string, sideLabel: string) {
    ...
    const searchRootMap = new Map<string, SearchRoot>();
    const addSearchRoot = (rootPath?: string, label?: string) => {
      if (!rootPath) return;
      const normalizedRoot = path.normalize(rootPath);
      if (!path.isAbsolute(normalizedRoot)) return;
      try {
    ...
    const searchStatsByKey = new Map<string, SearchStat>();
    ...
    const candidateByKey = new Map<string, vscode.Uri>();
    const candidateRootLabelByKey = new Map<string, string>();
    let duplicateCandidateCount = 0;
    const addCandidate = (uri: vscode.Uri, rootLabel?: string) => {
    ...
    const hasExcludedSegment = (filePath: string) => {
    ...
    const preferredSearchResults = await Promise.all(
      searchRoots.map(async (root) => {
        try {
          const uris = await vscode.workspace.findFiles(
            new vscode.RelativePattern(root.rootPath, preferredIncludePattern),
            excludePattern,
            5000
          );
          return { root, uris };
        } catch {
          return { root, uris: [] as vscode.Uri[] };
        }
      })
    );
    ...
    if (candidateByKey.size <= 1 && targetFileName) {
      ...
      const fallbackResults = await Promise.all(
        searchRoots.map(async (root) => {
          try {
            const uris = await vscode.workspace.findFiles(
              new vscode.RelativePattern(root.rootPath, "**/*"),
              excludePattern,
              12000
            );
            return { root, uris };
    ...
    const findSearchRootForFile = (filePath: string) => {
      const normalizedFile = normalizeSlashes(path.normalize(filePath)).toLowerCase();
      let matched: SearchRoot | undefined;
      for (const root of searchRoots) {
  ...
  function buildCompareRevUri(relPath: string, target: RevRangeTarget, rev: string) {
    const safeRelPath = relPath.replace(/\\/g, "/");
  ...
  function formatLocalFileLabel(filePath: string) {
  ...
  function formatCompareSideLabel(selection: CompareSideSelection) {
    ...
    return `${formatCompareSourceLabel(selection.source)} ${formatRevLabel(
      selection.rev
    )}`;
  }

  function buildCompareSideUri(relPath: string, selection: CompareSideSelection) {
    if (selection.source === "local") {
      if (!selection.localFilePath) {
  ...
  async function openCompareDiffForPath(
    relPath: string,
    leftSelection: CompareSideSelection,
    rightSelection: CompareSideSelection
  ) {
    const left = buildCompareSideUri(relPath, leftSelection);
    const right = buildCompareSideUri(relPath, rightSelection);
    const safeRelPath = relPath.replace(/\\/g, "/");
    const title = `${safeRelPath} (左:${formatCompareSideLabel(
      leftSelection
    )} -> 右:${formatCompareSideLabel(rightSelection)})`;
    await vscode.commands.executeCommand("vscode.diff", left, right, title);
  }

  async function compareFileHistory(
    item?: FileItem | RevFileItem | vscode.Uri | string
  ) {
    ...
    const defaultRight: RevRangeTarget =
      leftTarget === "branch" ? "trunk" : "branch";
    const rightTarget = await pickCompareSource(
      "选择右侧来源（分支/主线）",
      defaultRight
    );
    if (!rightTarget) return;
    let rightRev: string | undefined;
    try {
    ...
    try {
      await openCompareRevDiffForPath(
        relPath,
        leftTarget,
        leftRev,
        rightTarget,
  ...
  async function compareFileHistoryV2(
    item?: FileItem | RevFileItem | vscode.Uri | string
  ) {
    const relPath = resolveCompareFilePath(item);
    if (!relPath) {
    ...
    let leftSelection: CompareSideSelection;
    try {
      if (leftSource === "local") {
    ...
    const defaultRight: CompareSourceTarget =
      leftSource === "branch"
        ? "trunk"
        : leftSource === "trunk"
          ? "branch"
    ...
    let rightSelection: CompareSideSelection;
    try {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("svnMergeAnnotator.noop", () => {}),
    ...
    vscode.commands.registerCommand(
      "svnMergeAnnotator.compareDiffs",
      compareDiffs
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.refreshDiffCompare",
      refreshDiffCompare
    ),
    vscode.commands.registerCommand("svnMergeAnnotator.openRevDiff", openRevDiff),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.openRevMergeFile",
      openRevMergeFile
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.applyRevChangeToMerge",
      applyRevChangeToMerge
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.openMergeAtDiffLine",
      openMergeAtDiffLine
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.openMergeByHunkContext",
      openMergeByHunkContext
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.copyRevHunkDebugLog",
      copyRevHunkDebugLog
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.applyRevHunkToMerge",
      applyRevHunkToMerge
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.openMergeBySelection",
      openMergeBySelection
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.setRootPath",
      setRootPath
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.copyRootPath",
      copyRootPath
    ),
    vscode.commands.registerCommand(
      "svnMergeAnnotator.clearRootPath",
      clearRootPath
    ),
    vscode.commands.registerCommand(
    ...
    vscode.commands.registerCommand(
      "svnMergeAnnotator.compareFileHistory",
      compareFileHistoryV2
    ),
