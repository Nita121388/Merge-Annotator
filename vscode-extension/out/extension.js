...
function activate(context) {
    ...
    async function ensureAnnotationFilterAll(source) {
        const current = getAnnotationFilter();
        if (current === "all")
            return;
        try {
            const config = vscode.workspace.getConfiguration("svnMergeAnnotator");
            await config.update("annotationFilter", "all", vscode.ConfigurationTarget.Global);
